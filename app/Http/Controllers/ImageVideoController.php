...
class ImageVideoController extends Controller
{
    ...
    public function slideUpload(Request $request, $groupId = null)
    {
        ...
        //     'photos' => $photos,
        //     'storage' => Storage::disk('uploads-slides')->url('')
        // ]);
        ...
        foreach($photos as $k => $image) {
            if($image) {
                ...
                try {
                    ...
                    // echo 'Storage path: '.storage_path('/public/uploads/slides').'<br />';
                    // echo 'real path base path: '.getcwd().'<br />';
                    ...
                    Log::error('Failed to upload image. '.$err->getMessage());
                }
            }else{
                // remove null
                unset($photos[$k]);
