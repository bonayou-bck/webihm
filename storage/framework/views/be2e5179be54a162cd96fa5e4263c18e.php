...
<tbody>
  ...
  <tr>
    ...
    <td>
      ...
      <button type="button" data-id="<?php echo e($row->id); ?>" data-bs-toggle="tooltip" title=""
        class="btn btn-link btn-primary btn-lg" data-original-title="Edit Task">
        <i class="fa fa-edit"></i>
      </button>
      <form action="<?php echo e(route('admin.berita-delete', $row->id)); ?>" method="POST" class="d-inline" onsubmit="return confirm('Yakin ingin menghapus berita ini?')">
        <?php echo csrf_field(); ?>
...
<div class="modal fade" id="modalCreate" tabindex="-1" aria-hidden="true">
  <div class="modal-dialog modal-lg modal-dialog-scrollable">
    <form class="modal-content" method="POST" action="<?php echo e(url('admin/berita-create')); ?>" enctype="multipart/form-data">
      <?php echo csrf_field(); ?>
  ...
  document.querySelectorAll('.form-button-action .btn-link.btn-primary').forEach(function(btn){
    btn.addEventListener('click', async function(){
      ...
      try {
        ...
        document.getElementById('formEdit').action = `<?php echo e(url('admin/berita')); ?>/${id}`;
